...
while numRounds < 100:
    ...
    while startIndex < 1:
        ...
        if int(axieBreedCount) < 3 and float(axiePrice) < 500:
            ...
            if float(axiePrice) < 340 and recentAxieID != axieID:
                print(axiePrice)
                print(axieID)
                recentAxieID = axieID
                ...
                subject = "Axie " + axieID + " is available for $" + axiePrice + " right now!"
                message["Subject"] = subject
                message["From"] = me
                message["To"] = me
                
                offset = datetime.datetime.fromtimestamp(time.time()) - datetime.datetime.utcfromtimestamp(time.time())
                currentTime = datetime.datetime.utcfromtimestamp(time.time()) + offset
                currentTime = currentTime.strftime('%Y-%m-%d %I:%M:%S%p')
                

                axieInfo = ""
                axieInfo += "<div style='font-size: 18px;'><div style='border-bottom:1px solid black; font-weight: 700; font-size: 20px; margin-bottom: 20px;'>" + currentTime + "</div>"
                axieInfo += "<img style='width:300px;' src='" + axiePic + "'></img>"
                axieInfo += "<div style='display:table; font-weight:700; margin-bottom: 20px; border-bottom: 1px solid black;'><div style='display:table-cell; width:300px; vertical-align:top;'>"
                axieInfo += "<div style='margin-bottom: 4px;'>ID: " + axieID + "</div>"
                axieInfo += "<div style='margin-bottom: 4px;'>Name: " + axieName + "</div>"
                axieInfo += "<div style='font-size:24px;'>Price: " + axiePrice + "</div></div>"
                axieInfo += "<div style='display:table-cell'><div style='margin-bottom: 4px;'>Class: " + axieClass + "</div>"
                axieInfo += "<div style='border-left:1px solid black; border-top:1px solid black; border-right:1px solid black; padding-left:5px; padding-right:5px; font-weight:400;'>" + "HP: " + axieHP + " | Speed: " + axieSpeed + " | Skill: " + axieSkill + " | Morale: " + axieMorale + "</div>"
                axieInfo += "<div style='font-size:20px; padding-left:5px; border-left:1px solid black; border-bottom:1px solid black; border-right:1px solid black;'></div></div></div>"

                axieInfo += "<div style='font-size:20px; font-weight:700; margin-top: 10px;'>" + "Skill #1 Name: " + axieMove1 + " | Attack: " + str(axieMove1Attack) + " | Defense: " + str(axieMove1Defense) + " | Energy: " + str(axieMove1Energy) + "</div>"
                axieInfo += "<div>" + axieMove1Desc + "</div>"

                axieInfo += "<div style='font-size:20px; font-weight:700; margin-top: 10px;'>" + "Skill #2 Name: " + axieMove2 + " | Attack: " + str(axieMove2Attack) + " | Defense: " + str(axieMove2Defense) + " | Energy: " + str(axieMove2Energy) + "</div>"
                axieInfo += "<div>" + axieMove2Desc + "</div>"

                axieInfo += "<div style='font-size:20px; font-weight:700; margin-top: 10px;'>" + "Skill #3 Name: " + axieMove3 + " | Attack: " + str(axieMove3Attack) + " | Defense: " + str(axieMove3Defense) + " | Energy: " + str(axieMove3Energy) + "</div>"
                axieInfo += "<div>" + axieMove3Desc + "</div>"

                axieInfo += "<div style='font-size:20px; font-weight:700; margin-top: 10px;'>" + "Skill #4 Name: " + axieMove4 + " | Attack: " + str(axieMove4Attack) + " | Defense: " + str(axieMove4Defense) + " | Energy: " + str(axieMove4Energy) + "</div>"
                axieInfo += "<div>" + axieMove4Desc + "</div>"
                axieInfo += "<a href='https://marketplace.axieinfinity.com/axie/" + axieID +"'>" + "https://marketplace.axieinfinity.com/axie/" + axieID + "</a></div>"

                url = "https://marketplace.axieinfinity.com/axie/" + axieID
                webbrowser.register('chrome', None, webbrowser.BackgroundBrowser("C:\Program Files (x86)\Google\Chrome\Application\chrome.exe"))
                webbrowser.get('chrome').open(url)

                messageText = MIMEText(axieInfo,"html")
